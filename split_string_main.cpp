...
void MidString(char *result, char *chars, int StartPos , int EndPos)
{
	int len = strlen(chars);
	if((StartPos < len) && (chars != NULL))
	{
		strncpy(result, &chars[StartPos], EndPos - StartPos);
   ...
   if(str!= NULL)
   {
       ...
       for(;;)
       {
    	    ...
    	    if(*dup == token)
    	    {
                ...
                MidString(temp , str, beg , current_char);
    	    	if(temp == NULL)
    	    	{
    	    		break;
    	    	}
...
char **splitString2(char *str, char token)
{
   ...
   if(str!= NULL)
   {
       ...
       if(current_char < len)
       {
    	   ...
    	   MidString(other_temp  , str, current_char, len);
    	   if(other_temp != NULL)
    	   {
...
//
 //  ASplitString::MidString(mid, argv_0, 0, 6);
  // cout << mid <<endl;
